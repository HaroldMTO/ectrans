...
!    PRPNM(1:IU1,1:IU2) = F%RPNM(1:IU1,1:IU2)
    ...
    DO JMLOC=1,D%NUMP,NPRTRV
      ...
      DO JSETV=1,IPRTRV
        ...
        DO J=1,ILA
          DO I=1,IDGLU
            PRPNM(ISL+I-1,D%NPMS(IM)+IA+(J-1)*2) = REAL(S%FA(IMLOC)%RPNMA(I,J),JPRB)
          ENDDO
        ENDDO
        DO J=1,ILS
          DO I=1,IDGLU
            PRPNM(ISL+I-1,D%NPMS(IM)+IS+(J-1)*2) = REAL(S%FA(IMLOC)%RPNMS(I,J),JPRB)
          ENDDO
        ENDDO
      ENDDO
    ENDDO
...
IF(PRESENT(PLAPIN)) THEN
  ...
  PLAPIN(-1:R%NSMAX+2) = REAL(F%RLAPIN(:),JPRB)
ENDIF
